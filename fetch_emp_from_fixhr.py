...
def fetch_and_store_employees(token):
    ...
    for i, emp in enumerate(employees_data, 1):
        ...
        # Both possible URLs
        photo_url = emp.get("emp_profile_photo", "")
        s3_url = emp.get("emp_profile_s3_url", "")

        if not emp_code:
            continue

        # Try downloading from both (first photo, then s3)
        local_image_path, used_url = download_employee_image(emp_code, photo_url, s3_url)
...
def download_employee_image(emp_code, photo_url, s3_url):
    """
    Try downloading from emp_profile_photo first, then emp_profile_s3_url.
    Returns (local_image_path, used_url)
    """
    for image_url in [photo_url, s3_url]:
        if not image_url or not image_url.strip():
            continue
        try:
            img_resp = requests.get(image_url, timeout=15)
            ...
            local_image_path = os.path.join(IMAGE_DIR, filename)
            image.save(local_image_path, "JPEG", quality=90)
            return local_image_path, image_url  # ✅ return the URL that worked
        except Exception:
            continue
